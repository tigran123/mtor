...
\lettrine{\textcolor{lettrinecolour}{I}}{t} has been explained in the early chapters of \Title{Space, Time and Gravitation}
...
$(x_{1}, x_{2}, x_{3}, x_{4})$ and $(x_{1} + dx_{1}, x_{2} + dx_{2}, x_{3} + dx_{3}, x_{4} + dx_{4})$ in any coordinate\hyp{}system
is given by
\begin{multline*}
...
form a three\hyp{}dimensional world. Since $dy_{4}$~is zero for every pair of these
...
$y_{1}$,~$y_{2}$,~$y_{3}$ will appear to us as rectangular coordinates. The coordinate\hyp{}frames
...
Then in consequence of conditions~\Eq{(57.93)} and~\Eq{(57.94)} it is found that the Riemann-Christoffel
...
\Tag{(57.103)}
\]
The solution of this wave-equation is studied fully in~\SecRef{74}(d).
We have by~\Eq{(74.71)}
\[
...
\Tag{(57.107)}
\end{multline*}
This must be substituted in~\Eq{(57.105)} and the earliest non-vanishing terms picked out.
...
Riemann-Christoffel tensor, which corresponds to~\Eq{(52.6)}, viz.
\[
(\Star{B}^\epsilon_{\mu\nu\sigma})_\tau +
...
The field-theory (which we have seen in~\SecRef{76} to be essentially macroscopic) skirts round the excluded domain
...
in macroscopic terms; and it is the object of the action-theories of Weyl (\SecRef{90}) and Einstein (\SecRef{100})
